...
import { formatCompact } from "@/lib/formatDuration";

export default function Leaderboard({ data }: { data: Stats }) {
  const [sortBy, setSortBy] = useState<"userTimes" | "userYoinks">("userTimes");
  ...
  const leaderboard = useMemo(() => {
    ...
    return users.sort((a, b) =>
      sortBy === "userTimes" ? b.times - a.times : b.yoinks - a.yoinks,
    );
  }, [data, sortBy]);
    ...
    <div className="container mx-auto mt-5 px-4 sm:px-6 lg:px-32">
      ...
      <div>
        <button
          ...
          onClick={() => setSortBy("userTimes")}
        >
          Sort by Time
        </button>
        <button
          className="py-2 px-4 bg-gray-200 rounded hover:bg-gray-300 text-gray-600"
          onClick={() => setSortBy("userYoinks")}
        >
          Sort by Yoinks
        </button>
      </div>
